/**
 * create_income — Runtime implementation
 * Write tool: inserts income transaction.
 *
 * amount_cents always positive — sign encoded in kind='income'.
 * Idempotency: if idempotency_key exists for household, returns existing.
 */

import { query } from '../db.js';
import type { CreateIncomeResult } from '../types.js';
  ...
  try {
    ...
    const result = await query(
      `INSERT INTO transactions (id, household_id, kind, amount_cents, description, category_id, to_account_id, date, status, source_message_id, idempotency_key, created_at)
       VALUES ($1, $2, 'income', $3, $4, $5, $6, $7, 'confirmed', $8, $9, NOW())
       RETURNING id`,
