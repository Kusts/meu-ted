...
## Diagnóstico do bloqueio de auth (2026-09-09, evidência observada mas NÃO retida)
...
**Cadeia de diagnóstico** (NETDIAG/CSPDIAG/RDIAG* — sondas temporárias já removidas; observed-not-retained, ver nota):
...
| WS ×3 | "Sem conexão": diagnóstico aponta o proxy `/api/backend` perdendo o `x-e2e-test-id` → fixture não correlaciona o usuário de teste → 400 (Coder 1, observed-not-retained; divergência config×recomendação PENDENTE — ver seção WS×3) | MÉDIA |
...
1. **Autenticação (nó crítico, RESOLVIDO)**: o `authenticate()` clicava em "Registrar" inexistente e o fixture não enviava `Access-Control-Allow-Credentials` — journal vazio para `POST /auth/devices/register`, FAB nunca aparecia, 160/160 falhavam no bootstrap. Ver "Diagnóstico do bloqueio de auth" acima.
...
| 2 | CORS como causa principal, sem qualificar o caminho (HIGH) | ENDEREÇADO — causa válida só no caminho direto `:4010`; proxy `/api/backend` é same-origin; defeito do proxy = perda do `x-e2e-test-id` (diagnóstico Coder 1, observed-not-retained) |
...
| 9 | TransactionEditSheet snap-back + REC-05 falso-positivo (HIGH) | ENDEREÇADO (`b3804e0`+`ad93761`; regressão 2/2, consumidores 81/81; REC-05 endurecido; EDIT-SUB substituído por EDIT-CAT verde — `36da9a2`+`d9e0b0e`) |
...
| 11 | WS×3 sem causa isolada | Diagnóstico: proxy perde `x-e2e-test-id` → 400 (observed-not-retained); experimento formal com mocks estreitos+trace pendente (2º da fila) |
| 12 | Classificação operacional dos grupos (LOW/MEDIUM) | ENDEREÇADO — tabela "Restantes" reclassificada conforme o review |

Fila priorizada do Pi (status atual): 1º TransactionEditSheet+REC-05 — CONCLUÍDO; 2º WS×3 — diagnóstico (observed-not-retained), confirmação formal pendente; 3º G3-05 — pendente; 4º TED (corrigir rota mockada) — pendente; 5º UI-07/08 — pendente (verdes no run final; race a isolar se recidivar); 6º TX-06 — pendente; 7º admin-llm — pendente; 8º REP-01..04 — pendente. Nota: manter PAYSTMT/TRF como cobertura adicional até haver saldo/idempotência.

> Follow-up novo de contrato (achado R2 — priorizado logo após TransactionEditSheet/WS×3 por ser contrato financeiro): gap `subcategoryId` end-to-end — a API separa `subcategoryId` (`apps/api/src/types/domain.ts:56-69`, `apps/api/src/writes/types.ts:129-139`), a PWA não modela (`apps/pwa/src/lib/state/types.ts:3-18`, sem o campo), o PATCH do fixture ignora (`apps/pwa/e2e/fixture-api/server.ts:487-502`, sem `subcategoryId` no mutate) e o seed tx-1 não tem subcategoria (`apps/pwa/e2e/fixture-api/store.ts:215-218`). O EDIT-SUB foi removido por claim indevido (cobria "subcategoria" via `categoryId`/cat-4, que a UI agrega); o EDIT-CAT prova troca honesta de categoria de topo (cat-1→cat-2 com persistência) — o contrato dedicado `subcategoryId` segue como follow-up em item próprio.
...
- **EDIT-CAT ATIVO e verde** (EDIT-SUB removido por claim indevido de subcategoria): `e2e/specs/records-edit-cat.spec.ts` prova troca de categoria de topo com persistência (cat-1→cat-2, assert do payload do `PATCH`), verde em functional-mobile; `subcategoryId` end-to-end segue como follow-up.
- **WS×3 — diagnóstico (observed-not-retained)**: proxy `/api/backend` perde `x-e2e-test-id` → fixture 400 / "Sem conexão" (Coder 1); divergência config×recomendação PENDENTE de aplicação (decisão de config — migrar a base para 4010 invalida o baseline); experimento formal com mocks estreitos+trace segue como follow-up (ver seção WS×3).
