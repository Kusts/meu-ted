...
describe('T2.5 RED — postgres rotation store (SPEC §9 C4/C5)', () => {
  it('rotate resolves the predecessor scoped, inserts the hashed successor, windows the predecessor', async () => {
    ...
    const query = vi.fn(async (sql: string, args: unknown[]) => {
      ...
      if (sql.startsWith('SELECT')) {
        return { rowCount: 1, rows: [{ device_id: 'dev-old', household_id: HOUSEHOLD_ID, expires_at: null }] };
      }
    ...
    // Predecessor lookup is scoped by household (C5).
    ...
    const updates = calls.filter((c) => c.sql.startsWith('UPDATE device_tokens SET expires_at'));
  ...
  it('honors DEVICE_ROTATION_WINDOW_HOURS for the predecessor deadline', async () => {
    ...
    const updates = query.mock.calls.filter(([sql]: [string]) => sql.startsWith('UPDATE device_tokens SET expires_at'));
    expect(updates.length).toBeGreaterThanOrEqual(1);
    for (const [, args] of updates as Array<[string, unknown[]]>) {
      const deadline = new Date(args.find((a) => typeof a === 'string' && (a as string).includes('T')) as string).getTime();
