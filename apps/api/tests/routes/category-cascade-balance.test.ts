...
const getBalance = async (app: ReturnType<typeof buildTestApp>['app'], id: string): Promise<number> => {
  const res = await app.inject({ method: 'GET', url: '/accounts', headers: { 'x-device-token': TOKEN_A, 'idempotency-key': crypto.randomUUID() } });
...
describe('C-04 category cascade reverses balances (route)', () => {
  it('cascade restores the debited balance', async () => {
    ...
    const acc = await app.inject({
      method: 'POST', url: '/accounts', headers: { ...H, 'idempotency-key': crypto.randomUUID() },
      payload: { name: 'A', kind: 'bank', initialBalanceCents: 10_000 },
    });
    const accountId = acc.json().id as string;
    const cat = await app.inject({
      method: 'POST', url: '/categories', headers: { ...H, 'idempotency-key': crypto.randomUUID() },
    ...
    await app.inject({
      method: 'POST', url: '/transactions/expense', headers: { ...H, 'idempotency-key': crypto.randomUUID() },
      payload: { description: 'Lunch', amountCents: 1000, date: '2026-06-10', accountId, categoryId },
    });
    await app.inject({
      method: 'POST', url: '/transactions/income', headers: { ...H, 'idempotency-key': crypto.randomUUID() },
    ...
    const del = await app.inject({
      method: 'POST', url: `/categories/${categoryId}/delete`, headers: { ...H, 'idempotency-key': crypto.randomUUID() },
