...
export type DelegatedTurnClaims = {
  ...
  capabilities: string[]; jti: string; request: string; deviceId?: string; iat: number; exp: number;
};
export type DelegatedTurnInput = { actorId: string; workspaceId: string; role: DelegatedRole; capabilities: string[]; requestId: string; deviceId?: string };
...
/**
 * H-12: device binding — same contract as the Agent side
 * (`apps/agent/src/delegated-token.ts`). The deviceId is part of the signed
 * claims: tampering invalidates the signature, and the /rpc delegation
 * boundary verifies the SAME value (see routes/index.ts).
 */
...
export const createDelegatedTurnToken = async (input: DelegatedTurnInput, secret: string, nowMs = Date.now()): Promise<string> => {
  ...
  const payload = encodeJson({ iss: 'pi-agent', aud: 'pi-finance-api', sub: input.actorId, workspace: input.workspaceId, role: input.role, capabilities: input.capabilities, jti: randomUUID(), request: input.requestId, ...(input.deviceId !== undefined ? { deviceId: input.deviceId.trim() } : {}), iat, exp: iat + 300 });
