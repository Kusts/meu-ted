...
export async function listAccountsCommand(args: ListAccountsArgs, dryRun = false): Promise<CliResult> {
  ...
  return client.listAccounts(args.household);
}

export function parseListAccountsArgs(rawArgs: string[]): ListAccountsArgs {
  const args: ListAccountsArgs = { household: '' };
