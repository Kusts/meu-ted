...
export async function payBillCommand(args: PayBillArgs, dryRun = false): Promise<CliResult> {
  if (!args.household) return { success: false, reason: '--household é obrigatório' };
  if (!args.bill) return { success: false, reason: '--bill é obrigatório' };
...
export function parsePayBillArgs(rawArgs: string[]): PayBillArgs {
  const args: PayBillArgs = {
    household: '',
    bill: '',
    amountCents: 0,
    paymentDate: '',
  ...
  for (let i = 0; i < rawArgs.length; i++) {
    ...
    else if (arg === '--bill' && rawArgs[i + 1]) args.bill = rawArgs[++i];
