...
## Working Tree e Estado Atual (2026-09-22 `main@4e4e83c` — V4.1 CLOSURE MERGEADA; PWA deployada, Agent aguardando fix)
...
- **Produção (2026-09-22):** PWA Cloudflare **deployada em `4e4e83c`** (smoke SMOKE-05 validou `/api/build-info.gitSha == merge SHA`); Agent Cloudflare em `e0d5601` aguardando deploy (falha de bundle: agent-deploy não constrói `@pi-finance/llm-contracts` — fix em `fix/agent-deploy-contracts-build`); API VPS em `e0d5601` (deploy manual digest-based por `docs/reports/v4.1-release-bridge-execution.md` §9, exige autorização). `CLOUDFLARE_API_TOKEN` rotacionado em 2026-09-22 (token full-access do cofre `D:\projetos\cloudflare`; melhorar para token com escopo só de deploy).
