...
## Working Tree e Estado Atual (2026-09-17 `v4.1-hardening` — V4.1 IMPLEMENTADA e DEPLOYADA: API+PWA+Agent)
...
- **NÃO feito (gates humanos §3.4):** merge para `main` (branch pusheada, PR aberto para revisão); troca de visibilidade do repo; rotação de credenciais; rewrite de histórico; Release B do bearer (threshold D11: 0 eventos `auth.request.legacy_bearer_used` por 14 dias pós-Release A); cutover Canonical (operação separada pós release-bridge).
- **Produção (DEPLOY V4.1 COMPLETO 2026-09-17 sob autorização explícita do owner — release-bridge §3.3):** API container `pi-finance-api` = `pi-finance-api:v41-9ff0155` (gitSha `9ff0155…` exposto em `/health`, healthy), `_migrations` top = **V054** (V053/V054 aplicadas via migrate-job; backup pré-deploy `backups/pi-financeiro-pre-v41-9ff0155-20260917T154358Z.sql.gz`; rollback tag `pi-finance-api:rollback-pre-v41`). PWA Cloudflare = versão `c14c4dd9-9235-4620-a7a1-e1a7e584f556` (gitSha `d2db272…`, same-origin `/api/backend` proxy ativo, `/api/build-info` expõe release identity). Agent Cloudflare = versão `aea3c6f4-4593-434d-9e35-6837e2f5dd7e` (gitSha `234bf42…`; DO `WorkspaceAgent` deletado via migration v3 — dados de teste, autorização do owner; rollback `78c57fb9…` não recupera o storage). Postgres 15.18. Reconciliação read-only pós-deploy: 63 findings pré-existentes aguardando triagem humana (nenhum repair automático — `docs/reports/v4.1-release-bridge-execution.md` §4/§6).
- **Débitos conhecidos (seção honesta, pós-sessão de débitos 2026-09-17):** RESOLVIDOS nesta sessão — regeneração de `http-tools.ts` com auth no gerador; bulk payables single-tx keyed; banner de retry com mesmo `commandId`; suites integration auto-limpas (3× verdes no mesmo banco); `statement_timeout` de migrations (600s/60s); SHA-pinning completo (72 uses, gate estrito); sanitização public-safety (`--strict` PASS, 85→0); re-backfill de checksums executado em produção (boot sem `baseline_drift`); triagem dos 63 findings (`docs/reports/v4.1-reconciliation-triage.md` — 55 determinísticos propostos, 8 para revisão humana; **repairs NÃO executados**). RESTANTES — 63 findings aguardam autorização humana para repairs; billing do GitHub Actions (externo); stash drop + GC do objeto `11b82e8` (reflog expirado, gc em andamento; storage em packfile); migração Agent V1→V2 pending-ops (deferido: escopo de feature, rotas V1 seguem servidas); `StaleBanner` retry usa `reload()` por design — polish de refresh por domínio (follow-up UX); migração de `PWA_PROD_URL`/`AGENT_PROD_URL`/origens para repo-settings + fixtures `example.com` antes de remover as 23 allowlists não-detectoras; janela de compatibilidade localStorage TODO 2026-12-01 (ADR-011); `sharp@0.34.5` pinado; `.trivyignore` expira 2026-12-31.
