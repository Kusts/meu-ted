...
## Working Tree e Estado Atual (2026-09-21 `v4.1-hardening` — V4.1 CLOSURE IMPLEMENTADA, PR #11 aberto)
- **Baseline da closure:** `origin/main@7292dd8` (o estado `main@9910e42c` de 2026-09-18 é snapshot histórico — o histórico de `main` foi reorganizado após a V4.1). Branch `v4.1-hardening` contém a closure em 21 commits (`1bb241d..bf9005c`), 62 arquivos (+5188/−186). **PR #11 aberto aguardando merge humano** (branch protection já exige os checks).
...
- **Gates humanos restantes:** merge do PR #11; Release B (janela de 14 dias com 0 eventos `auth.request.legacy_bearer_used`); Canonical Cutover; decisão do dono sobre o finding `814332c4`; billing do GitHub Actions; itens future-dated (localStorage 2026-12-01, `sharp@0.34.5`, `.trivyignore` 2026-12-31); decisão de cobertura V1→V2 pending-ops.
- **Produção (NENHUM deploy nesta closure):** API container `pi-finance-api` healthy na VPS; PWA/Agent Cloudflare nos gitSha `9910e42…` verificados em 2026-09-18; confirmar `_migrations` de produção via `/health` antes de operar. Deploy manual segue runbook digest-based (`docs/reports/v4.1-release-bridge-execution.md` §9) e exige autorização humana.
