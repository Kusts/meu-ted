...
## Working Tree e Estado Atual (2026-09-16 `feat/meu-ted-v4-hardening@396a1c6`, V4 implementada — aguardando revisão/deploy)
- **Branch `feat/meu-ted-v4-hardening`:** SPEC V4 (`docs/MEU-TED-SPEC-HARDENING-SIMPLIFICACAO-E-DESCOMISSIONAMENTO-V4.md`) implementada por blocos A–K sobre a baseline de planejamento `5733cc8` (produção continua `3305152`). Entregues: mic capability-gated bidirecional (INV-08, XLT-01 em Chromium real), cookie-first com `offlineSubjectId` + `maxOfflineAge` (ADR-015 Opção C), device token V053 (hash SHA-256, lifecycle, rotação atômica single-use), WorkspaceAgent REMOVIDO (ARCH-V4-06a/06b PASS, ADR-016, binding/rotas/export fora), Undo com atomicidade PROVADA por crash injection nos schemas canônico e legacy (XLT-07 16/16 contra Postgres real — a suíte refutou a premissa F-1 da REV-V4-1 e a correção transacional foi aplicada nos dois schemas), borda/CSP hardening (`connect-src 'self'`, gate de localhost, `POST /api/csp-report` com sampling), observabilidade §24 (contrato fail-closed + endpoint `/client-events`, V054), lint real (Biome 4 apps, 0 erros), legado `.pi/` arquivado com I5 (contrato documental). Gates locais VERDES: typecheck, lint, testes (API 1694 / Agent 468 / PWA 1790 com XLTs 32/32), docs:lint, governance, write-policy 186/186, boundary, architecture, security (0 critical), build all 4 apps. **Nenhum deploy foi feito** — release, smoke, XLT-06 contra runtime publicado e observação 48h aguardam autorização do owner; CI remoto segue bloqueado por billing (débitos exaustivos em `docs/reports/meu-ted-v4-implementation-report.md` §9).
- **Branch `main` (baseline de produção):** Meu TED V3 hardening mergeado de `feat/meu-ted-v3-hardening` (merge `e1f55e7`; sync de lockfile `3305152`). SPEC V3 implementada por blocos A–F: Approval Tool Contract + validação por tool no propose, MutationDraft multi-turno com handoff idempotente (INV-09/10), PendingOperationCoordinator (botão + NL no mesmo Decision Service), recovery transacional (reemissão de attestation, TX1/executor/TX2, lease/reconciler — migration `V052`), grounding fail-closed, Mutation Effects Registry + receipts + MutationReconciler na PWA, card canônico fail-closed (`isActionablePendingOperationPresentation`), mic lifecycle + capability gate de anexos. Documentação: `docs/MEU-TED-SPEC-HARDENING-PONTA-A-PONTA-V3.md`, plano em `docs/superpowers/plans/2026-09-14-meu-ted-v3-hardening.md`, ADRs 012–014, relatório `docs/reports/meu-ted-v3-implementation-report.md`.
