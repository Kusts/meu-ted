...
| **Pós-Fase 1** | TED Agents SDK e configuração global de LLM | CONCLUÍDO ✅ | `FinanceChatAgent extends AIChatAgent`, chat global por workspace, tokens delegados anti-replay, migrações V034/V035 e configuração administrada por `ADMIN_EMAILS`. `openai-codex-subscription` permanece `experimental_blocked` até autorização e paridade comprovadas. |
| **Pós-Fase 1** | Workspaces e convites multiusuário | CONCLUÍDO ✅ | Ciclo de vida de workspaces, convites com rotação de token, ownership transfer e telas PWA, cobertos por commits `9efdfc3` a `012be1f`. |
