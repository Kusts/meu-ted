...
**Data:** 2026-09-07
**Status:** marca de produto renomeada nos textos; identificadores técnicos e decisões de infra pendentes do dono.
...
### 2.1 Pasta local `D:/projetos/pi-financeiro` → `meu-ted`
...
### 2.2 Repositório GitHub (`gh repo rename`)

- **Risco:** baixo. O GitHub mantém redirect do nome antigo; quebra apenas
  URLs hardcoded (badges, docs, Cloudflare Pages project linkage se criado via
  repo, webhooks com URL literal).
- **Passos seguros:**
  1. `gh repo rename meu-ted` (ou pela UI em Settings).
  2. Atualizar `remote origin` nos clones (`git remote set-url`).
  3. Conferir integrações conectadas (Cloudflare Pages, CI badges, Orca cards
     com link de repo) e trocar onde não houver redirect.
  4. Manter o nome antigo reservado (não recriar outro repo com ele).
