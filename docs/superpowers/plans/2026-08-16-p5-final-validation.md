...
- [x] **Step 1: RED — ordem e fail-closed**
...
- [x] **Step 2: Implementar subprocess runner**
...
- [x] **Step 1:** registrar `git status --short` e commit alvo; arquivos de implementação devem estar commitados/checkpointados e inventário P0 deve explicar qualquer WIP restante.
- [x] **Step 2:** `pnpm install --frozen-lockfile`.
- [x] **Step 3:** `git diff --exit-code -- pnpm-lock.yaml`.
- [x] **Step 4:** repetir frozen install em processo limpo/CI equivalente.
...
- [x] **Step 1:** `pnpm lint`.
- [x] **Step 2:** `pnpm docs:lint`.
- [x] **Step 3:** verificar no output de `pnpm lint` que os aliases `apps/api format:check`, `apps/pwa lint`, `apps/agent typecheck`, `apps/whatsapp-bridge lint`, `node --test scripts/*.test.mjs` e `pnpm docs:lint` foram executados; se algum não estiver composto, corrigir o alias root e repetir.
...
- [x] **Step 2:** verificar que produção, testes e scripts relevantes estão incluídos nos tsconfigs ou possuem checker próprio.
- [x] **Step 3:** rodar LSP diagnostics nos arquivos alterados suportados.
...
- [x] **Step 1:** remover/unset `DATABASE_URL`, `DATABASE_URL_TEST` e `DB_TEST_MARKER` do subprocesso unitário.
- [x] **Step 2:** `pnpm test:unit`.
- [x] **Step 3:** enumerar arquivos/tests/passed/skipped por workspace.
- [x] **Step 4:** qualquer tentativa de conexão externa falha o gate.
...
- [x] **Step 1:** `pnpm test:coverage`.
- [x] **Step 2:** produzir JSON/text summary por workspace ativo.
- [x] **Step 3:** exigir ≥80% statements, branches, functions e lines no escopo canônico; arquivos de produção sem testes aparecem como 0, não somem do relatório.
- [x] **Step 4:** mutation/adversarial sampling nos módulos críticos de auth, idempotência, pending operations e ownership.
...
- [x] **Step 1:** subir PostgreSQL descartável compatível com CI.
- [x] **Step 2:** criar marker server-side e exportar `DATABASE_URL_TEST`/`DB_TEST_MARKER` sem imprimir senha.
- [x] **Step 3:** aplicar migrations do zero e verificar fingerprint.
- [x] **Step 4:** `pnpm test:integration`.
- [x] **Step 5:** executar backup/restore rehearsal e integração de IDOR/idempotência/lock/replay/ownership.
- [x] **Step 6:** destruir somente o banco descartável identificado pelo marker.
...
- [x] **Step 1:** iniciar fixture/API/PWA/Agent locais com readiness e cleanup traps.
- [x] **Step 2:** `pnpm test:e2e` com workers/retries canônicos.
- [x] **Step 3:** repetir a suíte completa duas vezes consecutivas com retries 0.
- [x] **Step 4:** provar criar/editar/pagar, pending approve/reject, invite, workspace switch, chat, logout/revoke, offline/update e cross-workspace denial.
- [x] **Step 5:** preservar traces/screenshots/logs apenas para falhas e redigir dados sensíveis.
...
- [x] **Step 1:** `pnpm build:all` para todos os apps ativos no checkout final.
- [x] **Step 2:** confirmar que artefatos gerados não sujam o working tree versionado.
- [x] **Step 3:** validar o requisito histórico do bridge transitório em worktree temporário do tag `pre-g6-legacy-retirement-*`:
...
- [x] **Step 1:** `pnpm security:secrets` com history completo.
- [x] **Step 2:** `pnpm security:deps`.
- [x] **Step 3:** buildar imagens ativas e `pnpm security:containers`.
- [x] **Step 4:** executar SAST configurado; se alias estiver ausente, corrigir `security:check` e CI.
- [x] **Step 5:** revisar findings por ID/CVSS/fix/waiver; waiver não pode esconder CRITICAL e exige owner/expiry.
...
- [x] **Step 1:** `pnpm production:smoke:contract` prova localmente que o projeto Playwright é read-only e recusa writes.
- [x] **Step 2:** obter URL implantada da PWA e API a partir da topologia P3; não usar processos locais como produção.
- [x] **Step 3:** executar localmente contra produção:
...
- [x] **Step 4:** verificar health 200, auth deny esperado, assets/deep links, Agent health e referência de rollback; zero mutação/registro/login destrutivo.
...
- [x] **Step 2: Implementar cálculo determinístico**
...
- [x] **Step 5: Commit:** `docs: publish project closure rubric`.
...
- [x] **Step 1:** reviewer rastreia cada item G6/G7/VAL a evidence IDs e comandos.
- [x] **Step 2:** reviewer recalcula score sem confiar no total publicado.
- [x] **Step 3:** reviewer tenta ativar cada veto: capability gap, SQL boundary, cross-workspace, skip/failure, data risk, secret/CRITICAL, rollback, WhatsApp dependency, docs/runtime mismatch e consent violation.
- [x] **Step 4:** corrigir qualquer finding e reexecutar gates impactados.
- [x] **Step 5:** `git diff --check`, docs lint e working-tree inventory final.
- [x] **Step 6:** encerrar pi-task/Goal Mestre somente com ≥90, nenhum veto, P0–P5 verdes e relatório entregue.
