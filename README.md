...
|---|---|---|
| **API Autoritativa** (`apps/api`) | Fastify, TypeScript, PostgreSQL (pg), Zod | Hostinger VPS (`pi-stack`) |
| **PWA Web & Mobile** (`apps/pwa`) | Next.js 16, React, Tailwind CSS | Cloudflare Pages |
| **Assistente AI** (`apps/agent`) | Cloudflare Agents SDK, Durable Objects | Cloudflare Workers |
| **Bridge WhatsApp** (`apps/whatsapp-bridge`) | Node.js, Fastify, Evolution API | Hostinger VPS |
...
3. **PWA Canônica:** O cliente principal do usuário é a PWA hospedada na Cloudflare (`apps/pwa`).
...
# Validação do contrato de políticas de escrita
